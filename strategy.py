...
def opt_strategy(args):
    # opt the strategy for FL, such as FedAvg.
    assert args.strategy in candidates, 'You need to implement one custom FL strategy.'
    fl_strategy = getattr(strategy, args.strategy)

    class StrategyWithSteroids(fl_strategy):
        """Behaves just like <fl_strategy> but will log if an error occurs in a client.
        Else, because of how flower does the logging (+ Ray) errors won't be displayed,
        making the debugging process unnecessarily painful."""
    ...
    return StrategyWithSteroids

# You can custom one strategy inherated from the following one.
