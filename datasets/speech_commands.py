...
class PartitionedSPEECHCOMMANDS(SPEECHCOMMANDS):
    def __init__(self, data_path: Path, subset: str, transforms: list, classes: int, wav2fbank: bool = False,
                 bkg_volume: float = 0.1, bkg_frequency: float = 0.8):
        '''classes:
        ...
        self.bkg_volume = bkg_volume
        self.bkg_frequency = bkg_frequency

        assert classes in [35, 12], f'Speechcommands can only run in 35 or 12 classes mode, but you passed {classes}'
        ...
        self.classes_to_use = cls if classes==35 else CLASSES_12
    ...
    def split(self, ratio: float=0.5):
        """splits dataset into two. Arg `ratio` states the ratio of samples 
        that this datasets should keep. First part affects this object,
        the second part will be returned as a new object of this class"""
        num_samles_to_keep = int(len(self._walker)*ratio)
        this_walker, new_walker = self._walker[:num_samles_to_keep], self._walker[num_samles_to_keep:]
        self._walker = this_walker
        new_dataset = deepcopy(self)
        new_dataset._walker = new_walker
    ...
    def _blend_with_background(self, waveform):
        ...
        #TODO: should we be doing this as a torchaudio.transform ?
        background_volume = self.bkg_volume #  the default in [2] #! this seems to limit acc -- maybe lower is better?
        background_frequency = self.bkg_frequency # ratio of samples that will get background added in (as in [2])
...
def get_speechcommands_and_partition_it(destination_path: Path, version: int, split_federated: str="train"):
    ...
    assert split_federated in ['train', 'val', 'test'], f"`split_federated` should be one of ['train', 'val', 'test'] but got {split_federated}"
    ...
    # name of directory with partition varies depending on which set we want to explicitly split by client IDs
    partition_dir_name = "federated"
    if split_federated == "val":
        partition_dir_name = f"{partition_dir_name}_val"
    if split_federated == "test":
        partition_dir_name = f"{partition_dir_name}_test"

    for partition_dir in ['federated', 'federated_val', 'federated_test']:
        if partition_dir in cls_names:
            cls_names.remove(partition_dir)

    # now we generate the `federated` directory
    fed_dir = Path(whole_dataset._path)/partition_dir_name
    if not fed_dir.exists():
        ...
        if split_federated == 'train':
            ...
            ids = train_ids
        elif split_federated == 'val':
            val_files = set(_load_list(whole_dataset._path, "validation_list.txt"))
            files = [
                w
                for w in walker
                if HASH_DIVIDER in w and EXCEPT_FOLDER not in w and os.path.normpath(w) in val_files
            ]
            ids = val_ids
        else:
            test_files = set(_load_list(whole_dataset._path, "testing_list.txt"))
            files = [
                w
                for w in walker
                if HASH_DIVIDER in w and EXCEPT_FOLDER not in w and os.path.normpath(w) in test_files
            ]
            ids = test_ids

        print(f"Creating {len(ids)} {split_federated} partitions into {fed_dir}. Creating symlinks to data...")
...
def test(fed_dir: str, client_id: int, device: str, test_whole_dataset: bool = False):
    ...
    dataset = PartitionedSPEECHCOMMANDS(data_path, "training", transforms=raw_audio_to_AST_spectrogram(), classes=35)
...
if __name__ == "__main__":
    ...
    version = 2
    fed_dir = get_speechcommands_and_partition_it('./data', version=version, split_federated="val")
    print(f"{fed_dir = }")

    # just test everything works fine: this will load the data of the 99-th client and print its content
    test(fed_dir, client_id=99, device='cuda', test_whole_dataset=False)