...
class DistortImageFolder(data.Dataset):
    def __init__(self, path, alpha, img_ids, style, severity, transform=None, target_transform=None):
        self.path = path
        self.alpha = alpha
    ...
    def __getitem__(self, index):
        img_path = os.path.join(self.path, 'celeba', 'img_align_celeba', self.img_ids[index])
        img = Image.open(img_path).convert('RGB')
        if self.transform is not None:
            img = self.transform(img)
            img = self.style(img, self.severity)

        save_path = self.path + f'/federated_{self.alpha}' + '/transformed_data'

        if not os.path.exists(save_path):
            os.makedirs(save_path)

        save_path += img_path[img_path.rindex('/'):]
...
def get_style():
    ...
    d['Spatter'] = spatter
    d['Saturate'] = saturate
    style_key = np.random.choice(list(d.keys()))
    severity = np.random.randint(1, 6)
    
    return d[style_key], severity


def personalize_celeba(path, alpha, data):
    
    style, severity = get_style()
    distorted_dataset = DistortImageFolder(
        path=str(path), alpha=alpha, img_ids=data,
        style=style, severity=severity,
        transform=trn.Compose([trn.Resize(256), trn.CenterCrop(224)]))
    
    distorted_dataset_loader = torch.utils.data.DataLoader(
        distorted_dataset, batch_size=1024, shuffle=False, num_workers=8)
